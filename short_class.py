...
def buildWordVector(tokens, size):
    ...
    for word in tokens:
        try:
            #vec += nlp(word.decode("utf-8")).vector.reshape((1,size))* tfidf[word]
...
hist = model.fit(train_vecs_w2v, y_train, epochs=100, batch_size=32, verbose=1,callbacks=[tensorboard], 
