...
def load_w2v(w2v_dim):
    ...
    # path = '/data/szr207/AICS_Challenge_Data/glove.6B.' + w2v_dim + 'd.txt'  # 91.22% 159 epoch - better loss and accuracy
    wiki_vec = {}
    for i in tqdm(open(path)):
        m = i.split()
        word = m[0]
        vec = np.asarray(m[1:], dtype='float32')
